...
model differs from \ac{3pg} in that....\textbf{Difference between 3PG
  and SECRETS} To date, the use of
...
that coppicing is conducted. The \ac{3PG} model assumes a full month
...
estimation would overestimate the post-coppice regereration. These incremental
...
coppicing.  When a \ac{SRWC} is coppiced, there is a
...
assumed in the original model.  The seasonal timing of any root contribution is
moderated by comparing the actual \ac{NPP} to a potential \ac{NPP} the
tree would have a specified target \ac{LAI}.\textbf{This is unclear to
