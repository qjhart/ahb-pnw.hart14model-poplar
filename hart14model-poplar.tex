...
different poplar species.  To represent some of this variability, a
strategy was used where a set of default input values were modified on
a per variety basis to match field reported production.  These sets
were all run on the region, and the average production was
reported.
...
growing, interspecific Interamerican (\textit{Populus trichocarpa
  $\times$ P deltoides}) hybrid clones (Parameter sets Beaupré and
Raspalje), a native American \textit{P. trichocarpa} (Fritzi Pauley)
and one Euramerican reference clone \textit{P.~deltoides $\times$
  P.~nigra}, cv., (Robusta).
...
foliage and stem, and the inclusion of a coppicing model to allow for
regrowth.
...
proposed a set of parameterizations based on volume index (\VI) where
$VI = HD^2$, $H$ is height (m), and $D$ is tree diameter (m) at 22cm.
They described power relationships between \VI~and both above ground
dry matter and leaf area per stem. These can be used in a similar
manner to define \pfs, where \WS~is used to estimate \VI~for each stem
where \sps is the average number of stems per coppiced stump, and \VI
then used to predict leaf area per stem (\LA) and along with specific
leaf area (\SLA), \WF~and \pfs. As these relationships were defined
for each poplar variation in the field tests, the 3PG model was
modified to calculate \pfs~with these relationships, and the model run
with the parameters shown in Table~\ref{tab:pont-3pg}.  In that table,
the parameter set Base, is an otherwise unmodified base set of poplar
parameter assumptions.

\begin{table}%[p]
  \caption{\VI~parameter variations of \ac{3pg} among parameter sets.}
  \begin{tabular}{|l|c|c|c|c|c|}
    ...
    \hline 
  \end{tabular}
  \begin{flushleft}$DM=\sps(\beta_{DM} VI^{\gamma_{DM}})$, and $LA = \beta_{LA} VI^{\gamma_{LA}}$~\cite{pontailler97-volume-index}.
\end{flushleft}  
\label{tab:pont-3pg}
\end{table}

Two additional parameter sets HW1 and HW2 used the original
\pfs~formulation; $DM=\sps(C_{stem} \dbh^{P_{stem}})$, and $\pfs =
C_{pfs} \dbh^{P_{pfs}}$, with $\sps=2.8$, $C_{stem}=0.18$,
$P_{stem}=2.4$, $C_{pfs}=1.92$, $P_{pfs}=-1.162$ for HW1 and $\sps=1$,
$C_{stem}=0.08$, $P_{stem}=2.46$, $C_{pfs}=1.2$, $P_{pfs}=-0.77$ for
HW2 with a maximum allowable $\pfs=2$, when the coppiced trees small.
...
is coppiced, the root-shoot ratio no longer follows the expected ratio
for a given species and age. There is extra root mass when compared to
the aboveground biomass.  In the coppice submodel, this surfeit is
allowed to contribute to production, \RP~at each monthly timestep.
The seasonal timing of any root contribution is moderated by comparing
a monthly \ac{NPP} to a productivity (\NPPt)~the tree would have for a
target \ac{LAIt}. $\NPPt-\NPP$ serves as a limit on the root
...
conditions are not.  A maximum allowable monthly root contribution
fraction, \Rdp, limits the total contribution from the root excess
$\WR(\WR/\W-\pRx)$, where \pRx~is root fraction required by the
growing tree.  The value of \Rdp affects both the amount and the
timeliness where contributions are made.  In poplar, starch and sugar
content can be as much as 20\% of a coarse root, though this varies
seasonally~\cite{Regier2010}.

The monthly root contribution is $\RP =
min(\WR(\WR/\W-\pRx)\Rdp,\NPPt-\NPP)$ which simply says the root can
contribute up to $\NPPt-\NPP$ for growth, as long as there is excess
root mass available, limited to the \Rdp~fraction per month.
...
\label{tab:3pg-tree-coppice}
 ...
 Even with changes in the \pfs~calculations and the coppice model,
 contributions to the root mass are a fraction of \NPP ranging from a
 minimum to maximum as determined by the plant stress and nutrition,
 as in the original \ac{3pg} model.  These values, along with root
 turnover and litterfall are shown in
 table~\cite{tab:3pg-tree-allocate}.
...
\input{tree_productivity.tex}
\label{tab:3pg-tree-productivity}
 \end{table}

\begin{table}%[p]
\caption{3PG Growth Modifier Parameters}
\input{tree_modifiers.tex}
\label{tab:3pg-tree-modifiers}
 \end{table}

The parameterized model was then applied to the
